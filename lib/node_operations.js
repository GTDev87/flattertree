...
function addDataObjects(nodeObject, objects, callback, failCallback){
	async.each(_.pairs(objects), 
		function(pairs, iterCallback){
			...
			var parentNodePath = (nodeObject.path !== null ? nodeObject.path : ",") + nodeObject.name + ",";
			insertNode(dataName, parentNodePath, [], [], objectValue, function(){
				iterCallback()
			}, function(){
...
var deleteNode = function(path, callback, failCallback){
	...
	//need to delete ALL NODES UNDERNEATH THE PATH
	var pathParts = getPathParts(path);

	hasNode(pathParts.nodeName, pathParts.pathToNode, function(){
		Node.remove({path: path}, function(err){
...
var findNode = function(path, callback, failCallback){
	...
	Node.find({name: pathParts.nodeName, path: pathParts.pathToNode}, function(err, node){
		if(err){failCallback(err); }
		else{callback(node); }
	});
}
...
//     { $set: req.body}, 
//     function (err, topic) {
//       sendJsonToClient(res, err, "topic", topic, false)
//     }
//   )
// }
