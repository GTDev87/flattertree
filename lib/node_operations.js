...
function createNodeMetadata(name, parentNodePath, type, tags, unique){
	return {
		name: name,
		path: parentNodePath,
		tags: tags,
...
function createNode(nodeMetaData, data, callback, failCallback){
	...
	if(isArrayLike(data)){
		nodeMetaData["type"] = nodeMetaData["type"] ? nodeMetaData["type"] + " array" : "array" ;
	}
...
var findNodeDataObject = function(currentNode, callback, failCallback){
	Node.find({path: childPaths(currentNode), type: /^data/}, function(err, childNodes){
		if(err){failCallback(err); }
		else{
			async.map(
				childNodes, 
...
//////////////////////////////////visible functions//////////////////////////////////
var insertNode = function(name, parentNodePath, type, tags, unique, data, callback, failCallback){
	var nodeMetaData = createNodeMetadata(name, parentNodePath, type, tags, unique);
...
var updateNode = function(path, type, tags, unique, data, callback, failCallback){
	var pathParts = getPathParts(path);

	deleteNode(path, 
		function(){
			insertNode(pathParts.nodeName, pathParts.pathToNode, type, tags, unique, data, 
				function(){ callback(); }, 
				function(err){ failCallback(err); }
			);
		},
